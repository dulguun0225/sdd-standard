...
**In short.** When you learn something the spec does not yet say, you
change the spec and the code together, in the same PR (§5.2). Three
moves keep the record honest: supersede a requirement in place, append
a new R-id, or mark a dead one `WITHDRAWN` — never renumber, never
delete (§4.2). The rest of this guide walks the loop stage by stage,
then three full worked turns. Read a worked turn when you reach it; you
do not need them all on a first pass.
...
The artifacts run down the solid main path — spec, plan, tasks,
implementation, then the review phase. Dashed edges appear everywhere
the lifecycle loops back on itself. Four of those dashed edges are this
guide's subject:

- **spec ⇢ spike ⇢ spec** — what you cannot yet state as testable
  behavior, you prototype first, exempt from ceremony (§6.1);
- **review-notes ⇢ implementation** — a finding needs a fix: the
  review phase (§3.2) sends the work back before the item is marked
  done;
- **work item ⇢ hotfix ⇢ spec** — the emergency reversal: the code
  ships first, and the spec it drifted from is updated within
  5 working days (§6.2);
- **Done ⇢ Work item** — you learn from what shipped. The learning re-enters as
  the next work item. Where it changes behavior a spec
...
**The spec states current intent.** Two questions test every line
([reviewing-specs.md](reviewing-specs.md) asks them of whole
documents). Is it testable? Is it what we want *today*? Not: will this
stay correct once implemented. Amendment is normal and cheap. A team
whose specs never change is not writing great specs; it has stopped
updating them.

**Build against the artifacts.** The solid main path of the
diagram — Requirements → Design → Tasks before implementation, the
review phase after (§3.1).
...
a spec covers does not merge unless the same PR updates that
...
delete. Git history dates the amendment and names its author — the
diff is the whole record; no separate protocol exists. The
re-implementation was going to happen anyway. The amendment is the one
extra step, and it makes turn N+1 start from a record instead of from
memory.
...
the people are the walkthrough's
...
the learning arrived mid-implementation (Day 5), so the amendment
joined a PR already open. Here the learning arrives after Done, so
the turn opens a PR of its own. The shape is the same either way: one
PR carries the amended artifacts and the code (§5.2), the artifacts
are amended before the code that implements them — the Requirements →
Design → Tasks order a fresh feature follows (§3.1) — and the review
phase runs last.
...
| ---- | --- | ---- | ----- | --- | --- |
| **2026-07-17**, nine days after transfer-limit-alerts merged | A payroll client, then support, then **Nara** (PO) | A payroll run fired 40 transfers in one minute; each was rejected, each fired an event; the client got 40 identical sms and disabled the alert | A support ticket, checked against the `delivery_log` table (the data R-5's success criterion is measured over) | Nara queries the log — 40 deliveries in one minute, every one within R-5's 60 seconds. No defect | The service does exactly what the spec says, so no bug report captures it. Intent changed: "notify on every event" → "one per burst" |
...
| **2026-07-18** | **Bilguun** + agent draft | Two of §4.2's moves in one diff.<br>**Supersede R-5 in place** — add the clause "and no notification for that alert was delivered in the preceding 10 minutes", so it delivers within 60 seconds only outside that window.<br>**Append R-10** (new id) — "…whose most recent delivery is less than 10 minutes old, … record the event in the delivery log and shall not deliver a notification."<br>Edit the R-5 success criterion (99% within 60 s now measures the first event per window) | The turn's one PR, touching `specs/007-transfer-limit-alerts/spec.md` — the merged original. No copy, no v2 file: an amendment is a diff to the document | Edit the superseded text in place; append the new id after the highest ever used. The diff is the whole amendment record — git history keeps the date and the author, the PR keeps the why | **R-10, not R-9** — R-9 is taken (unsupported-channel, walkthrough Day 5); ids are never reused (§4.2). **Nothing renumbers** — R-1–R-4 and R-6–R-8 keep their ids, so every `[R-n]` still resolves |
| **2026-07-18** (same day — nothing waits between artifacts) | **Bilguun** + agent | `plan.md`: one new clause in §2's delivery flow — before calling notification-service, check `delivery_log` for a delivery under 10 minutes old — cited `[R-10]`.<br>`tasks.md`: append **T-8** — "Suppress delivery when the alert's most recent delivery is under 10 minutes old; record the suppressed event in `delivery_log`. Depends: T-5. [R-5] [R-10]." Evidence: burst-replay test — 40 events in one minute produce 40 delivery-log rows and exactly one notification | The same PR, now touching `plan.md` and `tasks.md` | Direct edits — **no `/speckit.*` command amends** (each drafts a whole document and would replace it wholesale). The agent re-checks R-id coverage both ways, that each task carries `[R-n]` (§5.1), states evidence, names dependencies | The artifacts state the behavior before the code exists — the same order a fresh feature follows (§3.1). **T-8, not T-7** — T-7 is taken (walkthrough's R-9 turn); T-ids are never reused |
| **2026-07-19** to **2026-07-20** | **Bilguun** + agent, task by task | The window check in the delivery path, the suppressed-event log write, and the burst-replay test named by T-8's evidence line | The same PR — alerts-service code and tests next to the amended artifacts | `/speckit.implement`; T-8's box is ticked when the burst-replay evidence exists, not before | §5.2 is satisfied by the PR's shape — and still binds: another unspecified case found now would be amended in this same PR |
| **2026-07-20** | The agent runs `speckit.sdd.review`; **Bilguun** resolves what it finds | `review-notes.md`, rewritten — a verdict per R-id, contract and task-evidence checks, and a drift section empty for the right reason (the change went through the spec) | The feature folder, inside the same PR | Every finding is resolved before the item is marked done (§3.2): fix the implementation, amend the artifact in this same PR, or record an explicit acceptance with a reason in the notes. This run has none to resolve, and the PR merges | The notes are the record turn N+1 starts from. Whether a colleague also reviews the PR is the team's own practice, outside the standard (§1) |

**What the record now shows.** A reader opening `spec.md` sees
current intent: R-5 with its window, R-9's unsupported-channel
rejection, R-10's suppression. Git history shows what changed, when,
and in which PR. The item is done — and the delivery log is already
collecting the data for the next turn. Turn N+1 starts from this
record, not from what anyone remembers.
...
so running one against a merged artifact would replace it
wholesale — the stable, ticked T-ids with it. Amendments of any size
are direct edits. The commands return when the loop returns something
that is not an amendment: a capability the existing spec does not
cover. That is this turn. Fictitious, like the turn above.
...
a spec covers, the team updates that spec within 5 working
